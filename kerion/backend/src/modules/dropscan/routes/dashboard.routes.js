  ...
  async (req, res) => {
    try {
      ...
      const [summaryRes, alertasRes, activeSessions, hourlyRes, operatorsRes, empresasRes] = await Promise.all([
        ...
        query(
          `SELECT s.id, COALESCE(u.nombre_completo, s.usuario_operador) as operador,
  ...
  async (req, res) => {
    try {
      ...
      const [dailyRes, totalRes, empresasRes, canalesRes, escaneadoresRes, hourlyRes] = await Promise.all([
        ...
        query(
          `SELECT COALESCE(u.nombre_completo, g.usuario_operador) as escaneador,
                  COUNT(DISTINCT g.tarima_id) as tarimas,
                  COUNT(g.id) as guias
           FROM tarimas t
    ...
    try {
      ...
      const result = await query(
        `SELECT
          ...
          e.nombre as empresa,
          c.nombre as canal,
          COALESCE(u.nombre_completo, s.usuario_operador) as operador,
          t.estado,
          t.cantidad_guias,
          t.fecha_inicio,
          t.fecha_cierre,
          ...
          g.posicion,
          g.timestamp_escaneo,
          COALESCE(gu.nombre_completo, g.usuario_operador) as operador_guia
        FROM tarimas t
        JOIN configuraciones e ON t.empresa_id = e.id
...
router.get('/escaneadores',
  ...
  async (req, res) => {
    try {
      ...
      const result = await query(
        `SELECT DISTINCT COALESCE(u.nombre_completo, g.usuario_operador) as escaneador
         FROM guias g
         JOIN tarimas t ON g.tarima_id = t.id
