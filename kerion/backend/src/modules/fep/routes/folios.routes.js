...
router.get('/:id/pdf',
  ...
  async (req, res) => {
    try {
      ...
      const writePageFooter = (num) => {
        const bottom = doc.page.height - 28
        doc.save()
        doc.fontSize(7).font('Helvetica').fillColor('#6b7280')
        doc.text(
          `${folio.folio_numero}  |  ${folio.empresa_nombre}  |  Estado: ${folio.estado}  |  Generado: ${generadoEn}  |  Página ${num}`,
          50, bottom, { width: 510, align: 'center' }
        )
        doc.restore()
      }
      ...
      // Write footer on first page (pageAdded doesn't fire for it)
      writePageFooter(1)
