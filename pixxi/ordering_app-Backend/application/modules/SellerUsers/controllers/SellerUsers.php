...
class SellerUsers extends BackendController
{
	//
	public $CI;
	...
	{
		parent::__construct();
		$this->load->model('SellerUser_model');
		$config = array(
			'protocol'  => 'smtp',
	...
	public function profile()
	{
		if($this->session->userdata("user_role_ids")!='1'){
			redirect('SellerUsers/sellerProfile');
		}
		authenticate();
		...
		$profile = $this->SellerUser_model->getUserData($user_id);


		if (!empty($profile)) {
			$data['profile'] = $profile;
			$loginRecords = $this->SellerUser_model->getLoginRecords($profile->user_email);
			...
			if ($this->input->post()) {
				...
				$result = $this->SellerUser_model->updateUser($user_id, $post);
	...
	public function account_settings()
	{
		...
		$user_id = $this->session->userdata("user_id");
		$profile = $this->SellerUser_model->getUserData($user_id);
		$languageList = $this->SellerUser_model->languageList();
		...
		if (!empty($profile)) {
			...
			if ($this->input->post()) {
				...
				if (!empty($code)) {
					...
					if ($checkResult) {
						$result = $this->SellerUser_model->updateUserSettings($user_id, $post);
						if (!empty($result)) {
							$this->session->set_flashdata('success_msg', 'Actualizado Correctamente');
						} else {
							$this->session->set_flashdata('error_msg', 'Actualización Incorrecta');
				...
				//die;				
				redirect('SellerUsers/account_settings');
			}
		} else {
			...
			if ($this->input->post()) {
				...
				if ($password_strength == '0') {
					...
					if ($new_password == $verify_password) {
						$current_password_chk = $this->SellerUser_model->checkCurrentPassword($current_password, $user_id);
						if (empty($current_password_chk)) {
							$this->session->set_flashdata('error_msg', 'Error! Old Password is Not Correct!');
						} else {
							$passwordChange = $this->SellerUser_model->changePassword($verify_password, $user_id);
				...
				//die;	
				redirect('SellerUsers/change_password');
			}
		} else {
			$this->session->set_flashdata('error_msg', 'User not available');
			redirect('SellerUsers');
	...
	public function loginRecordsList($email)
	{
		...
		echo $loginRecords = $this->SellerUser_model->getLoginRecords($email);
	}
	...
	public function listingCustomer($role_id = "2")
	{
		...
		$data['datas'] = $this->SellerUser_model->getSellerUsers($role_id);
		//pr($data['datas']); die;
		$result = array();
		...
		if ($this->input->post()) {
			...
			if ($_FILES['profile_image']['name'] != "") {
				...
				if (!$this->upload->do_upload('profile_image')) {
					// return the error message and kill the script
					$this->session->set_flashdata('error_msg', $this->upload->display_errors());
					redirect('SellerUsers/customerUpdate');
				}
				$image_data = $this->upload->data();
				$image = $image_data['file_name'];
			}
			...
			$result = $this->SellerUser_model->updateCustomer($post, $id);
			...
			redirect('SellerUsers/listingCustomer');
		}
	}

	public function remove($id)
	{
		$result = $this->SellerUser_model->customerRemove($id);
		return $result;
	}
	public function sellerProfile() {
        ...
        $this->user_images = realpath(APPPATH . '../assets/uploads/user_images/');
        $data['datas'] = $this->SellerUser_model->getCategoryList();
        $data['parent_categories'] = $this->SellerUser_model->getActiveCategoryList('0');
        if (!empty($id)) {
            $data['header']['site_title'] = 'Profile Update';
            $query = $this->SellerUser_model->getSellerData($id);
        ...
        if ($this->input->post()) {
            ...
            if (!empty($id)) {
                $post['modifiedBy'] =$this->session->userdata('user_id');
                $result = $this->SellerUser_model->updateSallerDetails($post, $post1, $post2, $post3, $id);
                $u_id = ($id);
            } 
            //******************Scheduling*******************************************//
            //$u_id = $result;
            if (!empty($this->input->post('schedule_day'))) {
                ...
                foreach ($this->input->post('schedule_day') as $k_d => $day) {
                    ...
                    $this->SellerUser_model->addUserSchedule($post_schedule);
                }
            }
            //***********************************************************************//
            if (!empty($result)) {
        ...
        $data['city_list'] = $this->SellerUser_model->getCityAllList();
        //pr($data);die;
        $data['query'] = $query;
        $data['seller_type'] = $this->SellerUser_model->getSellerTypeList();
        $this->render('admin/seller_profile', $data);
    }
	public function getAllchildcategoryData($pid, $selected_id = '') {


        $query = $this->SellerUser_model->getAllChildCategory($pid);
        //pr($query);die;
