	...
	<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
		...
		<div class="d-flex align-items-center py-1">
			<div class="">
				...
				<!--end::Svg Icon-->Añadir Cupón</button>
			...
			<div class="card-header border-0 pt-6">
				...
				<div class="card-title">
					...
					<div class="d-flex align-items-center position-relative my-1">
						...
						<!--end::Svg Icon-->
						<input type="text" data-kt-listing-table-filter="search" class="form-control form-control-solid w-250px ps-15" placeholder="Búsqueda" />
					</div>
					<!--end::Search-->
				</div>
				<!--begin::Card title-->
				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
					...
					<thead>
						...
						<tr class="text-start text-gray-400 fw-bolder fs-7 gs-0">
							...
							<th class="min-w-125px">Imagen</th>
                                                        <th class="min-w-125px">Nombre del Vendedor</th>
							<th class="min-w-125px">Código de Cupón</th>
							<th class="min-w-125px">Título del Cupón</th>
							<th class="min-w-125px">Cantidad Mínima</th>
							<th class="min-w-125px">Descuento</th>
							<th class="min-w-125px">Fecha de Validación</th>
					...
					if(!empty($datas)){
						foreach($datas as $k=> $rows)
						{
							...
							$edit_link = base_url('Coupons/save/'.$id);
							$delete_link = base_url('Coupons/remove/'.$id);
							$status_link = base_url('Coupons/statusChange/'.$id);
							$query = $this->Coupon_model->getCouponDetails($rows->id);
							$edit['query']=$query;
						...
						<tr id="tr_<?=$rows->id?>">							
							...
							<td>
								<img class="img-fluid w-125px" src="<?=$img?>" alt="<?=$rows->title?>">
                                                    ...
                                                    <td><?=$rows->sname?></td>
							<td>
								<a href="javascript:void()" class="text-gray-600 text-hover-primary mb-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit<?=$rows->id?>"><?=$rows->code?></a>
							</td>
							<td>
							...
							<td><?=$rows->min_order?></td>
                                                        <td><?=$rows->value?>  
                                                        <?php if($rows->discount_type=='percent') 
                                                        {
                                                            $dis='Porcentaje';
                                                        }if($rows->discount_type=='flat') 
                                                        {
                                                            $dis='Plano';
                                                        } ?>
                                                        <?= $dis;?><?php//=$rows->max_discount?></td>
							<td><?=($rows->enable_date!='0000-00-00')?date('d-M-Y',strtotime($rows->enable_date)):''?> To <?=($rows->disable_date!='0000-00-00')?date('d-M-Y',strtotime($rows->disable_date)):''?></td>
							<td data-filter="">
