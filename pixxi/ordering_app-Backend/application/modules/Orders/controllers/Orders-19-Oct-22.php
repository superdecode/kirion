...
class Orders extends BackendController
{
    ...
    public function listing()
    {
		...
		$data['datas'] = $this->Order_model->orderList($seller_id,$year,$start_date,$end_date);
  ...
  /*function convertpdf(){
   ...
   function testpdf()
	...
	public function earninglisting()
        ...
        public function vendor_order_listing($seller_id,$date)
    {
      authenticate();
      $search = $this->input->get();
      $seller_id = base64_decode($seller_id);
      $search_date = !empty($search['date'])?$search['date']:$date;
      $data['datas'] = $this->Order_model->sellerOrderList($seller_id,$search_date);
      $data['search']=$search;
