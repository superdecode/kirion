...
class Orders extends BackendController
{
	...
	public function saveRefund($id = '', $oid)
	{
		...
		$query = new stdClass();
		if (!empty($id)) {
			$data['header']['site_title'] = 'Detalles del reembolso';
			...
			$this->session->set_flashdata('error_msg', 'Ningún pedido seleccionado');
	...
	public function earninglisting()
	{
		...
		$this->render('admin/earninglisting', $data); 
                */
                authenticate();
                $data['header']['site_title'] = 'Informes de ganancias';
                if ($this->input->get()) {
                    $data['search'] = $this->input->get();
                } else {
                    $data['search'] = [];
	...
	public function casereport()
	{
		authenticate();
            ...
            $data['header']['site_title'] = 'Informes de ventas y caja';
            $result = array();
        ...
        public function kitchen_order($id = '', $orderno)
	{
		...
		$oid = base64_decode($id);
		//print_r($data);
		$data['header']['site_title'] = 'orden de cocina';
		$data['query'] = $query;
		$query = $this->barcode($oid, $orderno);
