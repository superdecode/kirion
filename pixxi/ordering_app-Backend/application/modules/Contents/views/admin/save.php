<!--begin::Form-->
<form class="form" id="update_form_<?=$query->id?>" method="POST" action="<?=base_url('Contents/save/'.$query->id)?>" id="" data-kt-redirect="<?=base_url('Contents/save/'.$query->id)?>" enctype="multipart/form-data">
	<!--begin::Modal header-->
	<div class="modal-header" id="kt_modal_edit_header">
		<!--begin::Modal title-->
		<h2 class="fw-bolder">Guardar contenido</h2>
		<!--end::Modal title-->
		<!--begin::Close-->
	...
	<div class="modal-body py-10 px-lg-17">
		...
		<div class="scroll-y me-n7 pe-7" id="kt_modal_edit_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_edit_header" data-kt-scroll-wrappers="#kt_modal_edit_scroll" data-kt-scroll-offset="300px">
			...
			<div class="fv-row mb-7">
				...
				<?php if(!empty($query->image)){?><img class="img-fluid w150" src="<?=base_url('assets/uploads/content_images/' . $query->image)?>" alt=""><?php } ?>
			</div>
			<div class="fv-row mb-7">
				<label class="required fs-6 fw-bold mb-2">Título</label>
				<input class="form-control form-control-solid" type="text" value="<?=$query->title?>" name="title" placeholder="Título" autocomplete="off" required>
			</div>
			<div class="fv-row mb-7">
				<label class="fs-6 fw-bold mb-2">Nº De Pedido</label>
				<input class="form-control form-control-solid" type="number" value="<?=$query->order_no?>" name="order_no" placeholder="Nº De Pedido" autocomplete="off" min="1" required>
			</div>
			<div class="fv-row mb-7">
				<label class="fs-6 fw-bold mb-2">Descripción</label>
				<textarea name="description" class="form-control form-control-solid editor" rows="5" placeholder="Descripción"><?=$query->description?></textarea>
			</div>
