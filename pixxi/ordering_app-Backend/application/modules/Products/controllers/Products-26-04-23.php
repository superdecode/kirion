...
class Products extends BackendController
{
			...
			if ($this->input->post('size') != '') {
				foreach ($this->input->post('size') as $k => $size) {
					...
					$result = $this->Product_model->saveSizeVariation($post2, $variation_id);
                        ...
                        /*$option_ids= explode(',',$post['option_ids']);
                        if (!empty($option_ids)) {
                            foreach($option_ids as $value){
                              ...
                              if (!empty($decode_id)) {
                                ...
                                foreach($this->input->post('att_options') as $k =>$val){
                                //$this->data['checks'] = $this->input->post('att_options');
                                $post3['attributes_options'] =$this->input->post('att_options')[$k];
                                $post3['attributes_id'] = $value;
                              //$post3['attributes_options'] =!empty($this->input->post('att_options')) ? implode(',', $this->input->post('att_options')) : '';
                              $result = $this->Product_model->saveProductAattributes($post3,$decode_id);
                                
                                }
                               //$k=$this->input->post('att_options.'[$value].''); 
	...
	public function attributes()
	{
		...
		$data['header']['site_title'] = 'Parameters Listing';
