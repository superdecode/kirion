...
<form class="form" id="update_form_<?=$query->id?>" method="POST" action="<?=base_url('Products/saveCategory/'.$query->id)?>" id="" data-kt-redirect="<?=base_url('Products/saveCategory/'.$query->id)?>" enctype="multipart/form-data">
	...
	<div class="modal-header" id="kt_modal_edit_header">
		<!--begin::Modal title-->
		<h2 class="fw-bolder">Guardar categoría</h2>
		<!--end::Modal title-->
		<!--begin::Close-->
	...
	<div class="modal-body py-10 px-lg-17">
		...
		<div class="scroll-y me-n7 pe-7" id="kt_modal_edit_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_edit_header" data-kt-scroll-wrappers="#kt_modal_edit_scroll" data-kt-scroll-offset="300px">
			...
			<!--<div class="fv-row mb-7">
				...
				<select class="form-select form-select-solid form-select-lg " tabindex="-1" aria-hidden="true" id="" name="parent_id">
					...
					?>
                        ...
                        <label class="required fs-6 fw-bold mb-2">Nombre del Vendedor</label>
                        
                        <select class="form-select" id="seller_ids" data-control="select2" data-placeholder="Select restaurants" data-allow-clear="true" multiple="multiple" name="seller_id[]" <?php if($this->session->userdata('user_id')=='1') {?>required <?php } ?>>
                        <?php
                            if(!empty($s_name)){
                                foreach($s_name as $k=>$s_name){
                                $catsArr = explode(',',$query->seller_id);
                            ?>
                        <option value="<?=$s_name->id?>" <?php if(in_array($s_name->id,$catsArr)){ echo 'selected'; } ?>><?=$s_name->seller_name?></option>
                        <?php
                                }
                        }
                        ?>
			...
			<div class="fv-row mb-7">
				<label class="required fs-6 fw-bold mb-2">Nombre de la Categoría</label>
				<input class="form-control form-control-solid" type="text" value="<?=$query->title?>" name="title" placeholder="Nombre de la Categoría" autocomplete="off" required>
			</div>
			<!--<div class="fv-row mb-7">
			...
			<div class="fv-row mb-7 d-none">
				...
				<input class="form-control form-control-solid" type="text" value="<?=$query->slug?>" name="slug" placeholder="Folio" autocomplete="off" >
			</div>
		
			<div class="fv-row mb-7">
				<label class="fs-6 fw-bold mb-2">Nº De Pedido</label>
				<input class="form-control form-control-solid" type="number" value="<?=$query->order_no?>" name="order_no" placeholder="Nº De Pedido" autocomplete="off" min="0" required>
			</div>
			<div class="fv-row mb-7">
			...
			<div class="fv-row mb-7 d-none">
				<label class="fs-6 fw-bold mb-2" for="signinSrEmail">Icono <small>(32x32)</small></label>
				<div class="">
					<div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="false" data-bs-toggle="modal" data-bs-target="#aizUploaderModal">
						<div class="input-group-prepend">
							<div class="input-group-text bg-soft-secondary font-weight-medium">Buscar</div>
