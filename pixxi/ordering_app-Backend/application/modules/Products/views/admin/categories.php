        ...
        <div class="d-flex align-items-center py-1">
            <div class="">
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add">
                    ...
                    <!--end::Svg Icon-->Agregar Categoría </button>
...
<div class="modal fade " id="kt_modal_add" tabindex="-1" aria-hidden="true">
    ...
    <div class="modal-dialog modal-dialog-centered mw-800px">
        ...
        <div class="modal-content">
            <!--begin::Form-->
            <?php
            $this->load->view('admin/saveCategory');
            ?>
            <!--end::Form-->											
        </div>
    </div>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<div class="d-flex align-items-center position-relative my-1 <?=$this->session->userdata('user_id')!='1'?'d-none':''?>">


<form class="form" method="POST" action="<?=base_url('Products/categories/')?>" >
<?php 
  $seller_id=$this->input->post('seller_id');
   
?>
<div class="row">
 <div class="col-lg-8 mb-10 row mt-7">    
                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
                    ...
                    <thead>
                        ...
                        <tr class="text-start text-gray-400 fw-bolder fs-7  gs-0">
                            ...
                            <th class="">Nombre de la Categoría</th>
                           
                            <th class="<?=$this->session->userdata('user_id')!='1'?'d-none':''?>">Nombre del Vendedor</th>
                            <th class="">Nº De Secuencia</th>
                    ...
                    <tbody class="fw-bold text-gray-600">
                        ...
                        if (!empty($datas)) {
                            foreach ($datas as $k => $rows) {
                                ...
                                $edit_link = base_url('Products/saveCategory/' . $id);
                                $delete_link = base_url('Products/removeCategory/' . $id);
                                $status_link = base_url('Products/statusChangeCategory/' . $id);
                                $query = $this->Product_model->getCategoryDetails($rows->id);
                                $edit['query'] = $query;

                                if (!empty($rows->banner_file_name)) {
                                    $banner = base_url('assets/uploads/files_manager/' . $rows->banner_file_name);
                                } else {
                                    $banner = base_url('assets/admin/media/illustrations/404-hd.png');
                                }
                                if (!empty($rows->icon_file_name)) {
                                    $icon = base_url('assets/uploads/files_manager/' . $rows->icon_file_name);
                                } else {
                                    $icon = base_url('assets/admin/media/illustrations/404-hd.png');
                                }
                                ?>
                                <tr id="tr_<?= $rows->id ?>">							
                                    <td>
                                    ...
                                    <td>
                                        <a href="javascript:void()" class="text-gray-600 text-hover-primary mb-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit<?= $rows->id ?>"><?= $rows->title ?></a>
                                    </td>
                                    <td class="<?=$this->session->userdata('user_id')!='1'?'d-none':''?>"><?= $rows->sname ?></td>
                                    <td><?= $rows->order_no ?></td>
                                  
                                    <td>
                                        <img class="img-fluid w-125px" src="<?= $banner ?>" alt="<?= $rows->title ?>">
                                    </td>
                                    <!--<td>
                                            <img class="img-fluid w-125px" src="<?= $icon ?>" alt="<?= $rows->title ?>">
                                    </td>-->
                                    <td data-filter="">
                                        <a href="<?= $status_link ?>" class="badge badge-light-<?= ($rows->status == '1') ? 'success' : 'danger' ?>"><?= ($rows->status == '1') ? 'Activa' : 'Suspendida' ?></a>
                                    </td>
                                    <td class="text-end">								
                                ...
                                <div class="modal fade " id="kt_modal_edit<?= $rows->id ?>" tabindex="-1" aria-hidden="true">
                                    ...
                                    <div class="modal-dialog modal-dialog-centered mw-800px">
                                        ...
                                        <div class="modal-content">
                                            <!--begin::Form-->
                                            <?php $this->load->view('admin/saveCategory', $edit); ?>
                                            <!--end::Form-->											
                                        </div>
                                    </div>
                                </div>
...
$this->load->view('admin/_cat_js', $this->data);
