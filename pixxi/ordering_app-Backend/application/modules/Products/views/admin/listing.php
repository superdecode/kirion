        ...
        <div class="d-flex align-items-center py-1">
            <div class="">
                <a href="<?= base_url('Products/save') ?>" class="btn btn-primary">
                    ...
                    <!--end::Svg Icon-->Agregar Producto</a>
            ...
            <div class="card-header border-0 pt-6">
                ...
                <div class="card-title">
                    ...
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <form class="form" method="POST" action="<?= base_url('Products/listing/') ?>">
                            <?php
                            $seller_id = $this->session->userdata('seller_id');
                            //$this->session->userdata('user_id')
                            ?>
                            <div class="row <?= $this->session->userdata('user_id') != '1' ? 'd-none' : '' ?>">
                                <div class="col-lg-10 mb-10 row mt-7">
                ...
                <div class="card-toolbar">
                    ...
                    <div class="d-flex justify-content-end align-items-center d-none" data-kt-listing-table-toolbar="selected">
                        <div class="fw-bolder me-5">
                            <span class="me-2" data-kt-listing-table-select="selected_count"></span>Seleccionado
                        </div>
                    ...
                    <thead>
                        ...
                        <tr class="text-start text-gray-400 fw-bolder fs-7  gs-0">
                            <th class="w-10px pe-2">
                                <div class="form-check form-check-sm form-check-custom form-check-solid me-3 ps-18">
                            ...
                            <th class="w-135px">Productos</th>
                            <th class="w-125px">Categoría</th>
                            <th class="w-125px">Precio</th>
                            <!--<th class="w-125px">Product information</th>-->
                            <th class="w-125px"> <?php if($this->session->userdata('user_id')==1) {?>Vendedor<?php } else {?>Acción<?php } ?></th>
                            <th class="w-70px">Estado</th>
                            <th class="w-90px">Acciones</th>
                        </tr>
                        <!--end::Table row-->
                    </thead>
                    <!--end::Table head-->
                    ...
                    <tbody class="fw-bold text-gray-600">
                        ...
                        if (!empty($datas)) {
                            foreach ($datas as $k => $rows) {
                                ...
                                $edit_link = base_url('Products/save/' . $id);
                                $delete_link = base_url('Products/remove/' . $id);
                                $status_link = base_url('Products/statusChange/' . $id);
                                ...
                                if (!empty($rows->image_default)) {
                                    $img = base_url('assets/uploads/files_manager/' . $rows->image_default);
                                } else {
                        ...
                        ?>
                                <tr id="tr_<?= $rows->id ?>">
                                    ...
                                    <td style="width: 100px;">
                                        <div class="row align-items-start">
                                            <div class="col">
                                                <div class="">
                                                    <?php if ($rows->variant_product == 1) { ?>
                                                        <a href="#" style="margin-top: -17px; margin-right: -13px;" class="accordion-button collapsed" data-bs-toggle="collapse" data-bs-target=".flush-collapseOne<?= $rows->id ?>" aria-expanded="false" aria-controls="flush-collapseOne<?= $rows->id ?>"></a>
                                                    <?php } ?>
                                                </div>
                                            </div>
                                            <div class="col">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <a href="<?= $edit_link ?>" class="symbol symbol-50px">
                                                <span class="symbol-label" style="background-image:url(<?= $img ?>);"></span>
                                            </a>
                                            <div class="ms-5">
                                                <a href="<?= $edit_link ?>" class="text-gray-800 text-hover-primary fs-5 fw-bolder"><?= $rows->title ?></a>
                                            </div>
                                        </div>

                                    </td>
                                    <td><?=$rows->cname ?> </td>
                                    <td> <?php if($rows->variant_product == 0) { ?>
                                    <?php echo $numberString = $fmt->format($rows->unit_price);?> <?php } ?></td>
                                   
                                    <td> <?php if($this->session->userdata('user_id')==1) {?> <?= $rows->sname ?><?php } else {?> 
                                        <?php if($rows->inventory_item==1) {?>
                                    <?= $rows->current_stock ?><?php } else { echo 'Non Inventory'; } }?></td>

                                    <td data-filter="">
                                        <a href="<?= $status_link ?>" class="badge badge-light-<?= ($rows->status == '1') ? 'success' : 'danger' ?>"><?= ($rows->status == '1') ? 'Activo' : 'Suspendido' ?></a>
                                    </td>
                                    <td class="">
                                        <a href="<?= $edit_link ?>" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                ...
                                <?php
                                if ($rows->variant_product == 1) {
                                    $img = base_url('assets/admin/media/illustrations/noproduct.jpg');
                                    $size = $this->Product_model->getSizevariationDetails($rows->id);
                                    if (!empty($size)) {
                                ?>
                                        <!--<tbody id="flush-collapseOne<?= $rows->id ?>" class="accordion-collapse collapse " aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">-->
                                        <?php
                                        foreach ($size as $k => $size_rows) {
                                        ?>
                                            <tr class="accordion-collapse collapse flush-collapseOne<?= $rows->id ?>">
                                                <td>&nbsp;
                                                </td>
                                                <td>
                                                    <div class="d-flex align-items-center ps-5">
                                                        ...
                                                        <div class="ms-5">
                                                            <a href="<?= $edit_link ?>" class="text-gray-800 text-hover-primary fs-5 fw-bolder"><?= $size_rows->variation_name ?></a>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>&nbsp;
                                                </td>
                                                <td><?= $fmt->format($size_rows->price); ?></td>
                                                <td><?php//= $rows->sname ?> </td>
                                                <td data-filter="">
                                                    &nbsp;
                                                </td>
                                                <td class="">
...
<!--end::Post-->
<?php
$up = base_url('assets/admin/media/illustrations/arrow_up.png');
$down = base_url('assets/admin/media/illustrations/arrow_down.png');
?>
<style>
...
$this->load->view('admin/_listingjs', $this->data);
?>