				...
				<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
					...
					if(!empty($datas)){
						foreach($datas as $k=> $rows)
						{
							...
							$query = $this->User_model->getUserData($rows->user_id);
							$edit['query']=$query;							
					?>
						<tr id="tr_<?=$rows->user_id?>">							
							...
							<td>
								<a href="<?=$edit_link?>" class="badge badge-light-success"><?=$rows->user_login_id?> </a>
							</td>
							<td>
								<?=$rows->full_name ?>
							</td>
							<td><?=$rows->email?><br><?=$rows->phone_no?></td>
							<td data-filter="">
							...
							<td class="text-end">								
								...
								<button type="button" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" onclick="return removeData('<?=$rows->user_id?>');">
