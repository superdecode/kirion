    ...
    <div data-kt-place="true" data-kt-place-mode="prepend" data-kt-place-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-left me-3 flex-wrap mb-5 mb-lg-0 lh-1">
      ...
      <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
        ...
        <li class="breadcrumb-item text-muted">
          <a href="<?= base_url('Messages/listing') ?>" class="text-muted text-hover-primary">Lista de Mensajes</a>
        </li>
        <!--end::Item-->
    ...
    <div class="d-flex align-items-center py-1">
      <div class="">
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add">
          ...
          <!--end::Svg Icon-->Agregar Mensaje</button>
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
          ...
          <thead>
            ...
            <tr class="text-start text-gray-400 fw-bolder fs-7  gs-0">
              ...
              <th class="min-w-125px">Mensaje</th>
          ...
          <tbody class="fw-bold text-gray-600">
            ...
            if (!empty($datas)) {
              foreach ($datas as $k => $rows) {
                ...
                $edit_link = base_url('Messages/save/' . $id);
                $delete_link = base_url('Messages/remove/' . $id);
                $status_link = base_url('Messages/statusChange/' . $id);
                $query = $this->Message_model->getMessageDetails($rows->id);
                $edit['query'] = $query;

                if (!empty($rows->image)) {
                  $img = base_url('assets/uploads/content_images/' . $rows->image);
                } else {
            ...
            ?>
                <tr id="tr_<?= $rows->id ?>">
                  ...
                  <td>
                    <a href="javascript:void()" class="text-gray-600 text-hover-primary mb-1" data-bs-toggle=" <?=$this->session->userdata('user_role_ids')!='1'?'':'modal'?>" data-bs-target="#kt_modal_edit<?= $rows->id ?>"><?= $rows->seller_name ?></a>
                  </td>
                  <!--<td>
<a href="javascript:void()" class="text-gray-600 text-hover-primary mb-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit<?= $rows->id ?>"><?= $rows->title ?></a>
                  ...
                  <td>
                    <?php $middle = strtotime($rows->addedOn); ?>
                    <?php echo $old_date = date('d M Y', $middle); ?>
                  </td>
                  <td data-filter="">
...
$this->load->view('admin/_message_list_js', $this->data);
?>