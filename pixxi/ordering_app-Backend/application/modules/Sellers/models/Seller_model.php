...
class Seller_model extends CI_Model
{
	...
	public function getQrCode()
	{
		...
		$this->db->order_by("id", "desc");
                if ($this->session->userdata('user_role_ids') != '1') {
                    $this->db->where("qr_scan.seller_id", $this->session->userdata('seller_id'));
                }
		$this->db->join('seller', 'seller.id = qr_scan.seller_id', 'LEFT');
		return $datas = $this->db->get()->result();
	}
	public function userStatusChange($id,$login_id)
	{
	...
	public function updateSallerDetails($post, $post1, $post2, $post3, $user_id)
	{
		...
		$query_email = $this->Seller_model->get_email($user_id);
		$email_id = $query_email->email;
		$this->db->where('login_id', $email_id);
	...
	public function saveSeller($post, $post1, $post2, $post3, $id = "")
	{
		if (!empty($id)) {
			$this->db->where('id', $id);
			$res = $this->db->update('seller', $post);
			...
			$res1 = $this->db->insert('users', $post1);
			$user_id = $this->db->insert_id();
			$post2['user_id'] = $user_id;

			$post3['user_id'] = $user_id;

			$this->db->insert('user_profiles', $post2);
			//echo $this->db->last_query();	die();
			$this->db->insert('user_settings', $post3);
		}
		return $seller_id;
	}
	...
	public function getSellerTable()
	{
		...
		$this->db->from('sellers_table');
                if ($this->session->userdata('user_role_ids') != '1') {
                        $this->db->where("sellers_table.seller_id", $this->session->userdata('seller_id'));
                }
	...
	public function get_email($id)
	{
        ...
        public function getNewUserDetails($email)
	{
		$this->db->select('users.is_first_login');
		$this->db->from('users');
		$this->db->where("users.login_id", $email);
		$datas = $this->db->get()->row();
		//echo $this->db->last_query(); die();
		return $datas;
	}
        
        public function getAllUserDetails($login_id)
	{
		$this->db->select('users.login_id,users.status,users.role_ids,user_profiles.fname,user_profiles.lname,');
		$this->db->from('users');
		$this->db->where("users.login_id", $login_id);
		$this->db->join('user_profiles', 'users.id = user_profiles.user_id', 'LEFT');
		$this->db->where("user_profiles.is_main", '1');
