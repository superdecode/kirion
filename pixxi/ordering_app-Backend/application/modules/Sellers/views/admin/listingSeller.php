                ...
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
                    ...
                    <thead>
                        ...
                        <tr class="text-start text-gray-400 fw-bolder fs-7 gs-0">
                            ...
                            <th class="">Imagen de la tienda</th>

                            <th class="">Nombre de la Tienda</th>
                            <th class="">Teléfono de la Tienda</th>
                            <th class="">Dirección de la Tienda</th>
                            <th class="">Fecha y Hora de Creación</th>
                            <th class="">Estado</th>
                    ...
                    <tbody class="fw-bold text-gray-600">
                        ...
                        if (!empty($datas)) {
                            foreach ($datas as $k => $rows) {
                                ...
                                $email = base64_encode($rows->email);
                                //$edit_link = base_url('Sellers/save/'.$id);
                                $edit_link = base_url('Sellers/save/' . $id);
                                $delete_link = base_url('Sellers/remove/' . $id);
                                $status_link = base_url('Sellers/statusChangeSeller/' . $id.'/'.$email);
                                $get_is_new =  $this->Seller_model->getNewUserDetails($rows->email);
                                $edit['query'] = $query;
                        ...
                        ?>
                                <tr id="tr_<?= $rows->id ?>">
                                    ...
                                    <td><?= date('d M Y ', strtotime($rows->addedOn)) . '<br>' . date('h:i A ', strtotime($rows->addedOn)) ?></td>
                                    <td data-filter="">
                                        <a href="<?= $status_link ?>" class="badge badge-light-<?= ($rows->status == '1') ? 'success' : 'danger' ?>" onclick="return confirm('Do you Want to change approval status??')"><?= ($rows->status == '1') ? 'Aprobado' : 'Pendiente de Aprobación' ?></a>
                                    </td>

                                    <td class="text-end">
                                            <?php if($rows->status==0) {?>
                                         <img class="img-fluid w-50px" src="<?=base_url('assets/uploads/user_images/new_seller.png')?>" alt="new seller">
                                        <?php } ?>
...
$this->load->view('admin/_seller_listing_js', $this->data);
?>