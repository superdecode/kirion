        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_listing_table">
                ...
                <thead>
                        ...
                        <tr class="text-start text-gray-400 fw-bolder fs-7 gs-0">
                                ...
                                <th class="min-w-125px">Móvil</th>
                                <th class="min-w-125px">Mensaje</th>
                                <th class="min-w-125px">Experiencia</th>
                                
                                <th class="min-w-125px">Imagen</th>
                                <th class="min-w-125px">Tipo de Comentarios</th>
                               
                                <th class="min-w-50px">Estado</th>
                                <th class="text-end min-w-50px">Acciones</th>
                        </tr>
                        <!--end::Table row-->
                </thead>
                <!--end::Table head-->
                ...
                if(!empty($datas)){
                        foreach($datas as $k=> $rows)
                        {
                                ...
                                $edit_link = base_url('Contacts/save/'.$id);
                                $delete_link = base_url('Contacts/remove/'.$id);
                                $status_link = base_url('Contacts/statusChange/'.$id);
                                $query = $this->Contact_model->getFeedbackDetails($rows->id);
                                $edit['query']=$query;
                                 if (!empty($rows->image)) {
                                        $img = base_url('assets/uploads/feedback_images/' . $rows->image);
                                } else {
                ...
                <tr id="tr_<?=$rows->id?>">							
                        ...
                        <td><a href="javascript:void()" class="text-gray-600 text-hover-primary mb-1" data-bs-toggle="modal" data-bs-target="#kt_modal_edit<?=$rows->id?>"><?=$rows->email?></a></td>
                         <td>
                            <?= $rows->phone?>
                        </td>
                       <td><?=$rows->message?></td>
                       ...
                       <td>
                        ...
                        <td><?=$rows->feedback_type?></td>
                      <td>
                        <?php if($rows->feedback_status=='solved')
                        {
                          $feed_status='Resuelto';  
                          
                        }if($rows->feedback_status=='ongoing')
                        {
                          $feed_status='Actual';  
                          
                        }if($rows->feedback_status=='closed')
                        {
                          $feed_status='Cerrado';  
                          
                        }if($rows->feedback_status=='open')
                        {
                          $feed_status='Apertura';  
                        }
                        ?>
                        <?=$feed_status?></td>
