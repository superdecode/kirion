...
class Product_model extends CI_Model {
	...
	{
		$data= array();
		$this->db->select('products.*,seller.seller_name as sname,product_categories.title as cname');
		$this->db->from('products');
		$this->db->where("products.deleted","0");
	...
	public function getCategoryList($parent_id="")
	{
		...
		if(!empty($parent_id)){ $this->db->where("product_categories.parent_id",$parent_id); }
		$this->db->order_by("id", "desc");
	...
	{
		$this->db->select('product_brands.*,banner_upload.file_original_name,banner_upload.file_name as banner_file_name,banner_upload.extension,icon_upload.file_original_name as icon_original_name,icon_upload.file_name as icon_file_name,icon_upload.extension as icon_extension');
		$this->db->from('product_brands');
		$this->db->where("product_brands.deleted","0");
		$this->db->order_by("id", "desc");
		$this->db->join('uploads as banner_upload', 'banner_upload.id = product_brands.banner' , 'LEFT');
		$this->db->join('uploads as icon_upload', 'icon_upload.id = product_brands.icon' , 'LEFT');
		$datas = $this->db->get()->result();
		//echo $this->db->last_query();
		return $datas;
	...
	public function getActiveBrandList()
	{
		...
		$this->db->from('product_brands');
		$this->db->where("product_brands.deleted","0");
		$this->db->where("product_brands.status","1");
		$this->db->order_by("product_brands.title", "asc");
		$this->db->join('uploads as banner_upload', 'banner_upload.id = product_brands.banner' , 'LEFT');
		$this->db->join('uploads as icon_upload', 'icon_upload.id = product_brands.icon' , 'LEFT');
		$datas = $this->db->get()->result();
		//echo $this->db->last_query();
	...
	public function saveBrand($post,$id=""){
		if(!empty($id)){
			$this->db->where('id', $id);
			$res = $this->db->update('product_brands', $post);
		}else{
			$res = $this->db->insert('product_brands', $post);
		}
		return $res;
	}
	...
	public function getBrandDetails($id){
		...
		$this->db->from('product_brands');
		$this->db->where("product_brands.id",$id);
		$this->db->order_by("id", "desc");
		$data = $this->db->get()->row();
		return $data;
	}
	public function statusChangeBrand($id)
	{
		...
		$this->db->where("id", $id);
		return $this->db->update("product_brands");	
	}
