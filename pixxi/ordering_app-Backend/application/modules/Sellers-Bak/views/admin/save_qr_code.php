<!--begin::Form-->
<form class="form" id="update_form_<?=$query->id?>" method="POST" action="<?=base_url('Sellers/add_qr_data/')?>" id="" data-kt-redirect="<?=base_url('Sellers/add_qr_data/')?>" enctype="multipart/form-data">
	<!--begin::Modal header-->
	<div class="modal-header" id="kt_modal_edit_header">
		<!--begin::Modal title-->
		<h2 class="fw-bolder">Save Qr Code</h2>
		<!--end::Modal title-->
		<!--begin::Close-->
	...
	<div class="modal-body py-10 px-lg-17">
		...
		<div class="scroll-y me-n7 pe-7" id="kt_modal_edit_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_edit_header" data-kt-scroll-wrappers="#kt_modal_edit_scroll" data-kt-scroll-offset="300px">
			...
			<div class="fv-row mb-7">
                            ...
                            <select name="seller_id" id="seller_id" required="true" class="form-select">
                                <option value="">Select Seller Name</option>
                               <?php
                                if(!empty($seller_name)){
                                        foreach($seller_name as $k=>$seller_name_list){
                                ?>
                                <option value="<?=$seller_name_list->id?>"><?=$seller_name_list->seller_name?></option>
                               <?php } }?>
                            </select>
			</div>
